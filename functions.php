...
function add_individual_stylesheet_script()
{
    ...
    elseif (is_post_type_archive('event')) {
        ...
        // 体験の一覧ページのCSS（farming_area.css）を読み込む（当初の地域別用）
        wp_enqueue_style('awa_nolife-search-area-event', get_template_directory_uri() . '/assets/css/farming_area.css', array(), false);
    } elseif (is_search()) {

        // 体験の詳細検索のCSS（farming_search.css）を読み込む
        wp_enqueue_style('awa_nolife-search-event', get_template_directory_uri() . '/assets/css/farming_search.css', array(), false);
    ...
    elseif (is_post_type_archive('farmer')) {
        ...
        wp_enqueue_style('awa_nolife-archive-farmer', get_template_directory_uri() . '/assets/css/farmer.css', array(), false);

        //farmer.jsを読み込み
        wp_enqueue_script('awa_nolife-farmer', get_template_directory_uri() . '/assets/js/farmer.js', '', '', true);
    } elseif (is_singular('farmer')) {

        // 農家さんの個別ページのCSS（farmer_content.css）を読み込む
        wp_enqueue_style('awa_nolife-single-farmer', get_template_directory_uri() . '/assets/css/farmer_content.css', array(), false);

        // 野菜の個別ページのJS（farmer_content.js）を読み込む
        wp_enqueue_script('awa_nolife-farmer_content-script', get_template_directory_uri() . '/assets/js/farmer_content.js', '', '', true);
    }
    //----------------------
    //  02-F:農ライフ記事
    //----------------------
    ...
    elseif (is_page('newslist')) {
        ...
        //ニュース一覧のCSS（news.css）を読み込む
        wp_enqueue_style('awa_nolife-news', get_template_directory_uri() . '/assets/css/news.css', array(), false);
    } elseif (is_single()) {

        //ニュースの個別ページのCSS（news-content.css）を読み込む
        wp_enqueue_style('awa_nolife-news-content', get_template_directory_uri() . '/assets/css/news_content.css', array(), false);
    }
}
